...
#ifndef MF_AP_DSGDPP_FACTORIZATION_H
#define MF_AP_DSGDPP_FACTORIZATION_H
...
bool checkBlockingDsgdPp(const DistributedMatrix<M1>& v, const DistributedMatrix<M2>& w,
		const DistributedMatrix<M3>& h) {
	if (2*v.blocks1() != v.blocks2()) return false; // d-by-d condition
 ...
 * (1) V is blocked d x 2d,
 * (2) W is conformingly blocked d x 1,
 * (3) H is conformingly blocked 1 x 2d,
 ...
 * (6) each rank holds exactly 2t blocks of H.
 */
template<typename Data = double, typename Factor = double>
struct DsgdPpFactorizationData : public DistributedFactorizationData<Data,Factor> {
public:
	...
	DsgdPpFactorizationData(
			const DV& dv,
			DW& dw,
			DH& dh, unsigned tasksPerRank = 1)
	: Base(dv, dw, dh, tasksPerRank) {
		if (!checkBlockingDsgdPp(dv, dw, dh)) RG_THROW(rg::InvalidArgumentException, "");
	}

	DsgdPpFactorizationData(DsgdPpFactorizationData<Data,Factor>& o)
	: Base(o) {
	}

	DsgdPpFactorizationData(mpi2::SerializationConstructor _)
...
MPI2_SERIALIZATION_CONSTRUCTOR2(mf::DsgdPpFactorizationData);
