...
int main(int argc, char* argv[]) {
	...
	mf_size_type blocks2 = world.size() * tasksPerRank * 2;
...
//	Loss loss((NzslLoss()), Nzl2Loss(lambda));
	...
	// start mf library
	registerTask<mf::detail::DsgdPpTask<Update,Regularize> >(true);
	mfStart();
	...
	if (world.rank() == 0)
	{
		...
		// initialize the DSGD++
		Timer t;
		DsgdPpRunner dsgdRunner(random);
		DsgdPpJob<Update,Regularize> dsgdJob(dv, dw, dh, update, regularize, order,
				stratumOrder, tasksPerRank);
		...
		// print the test loss
		DsgdPpFactorizationData<> testData(dvTest,dw,dh,tasksPerRank);
		...
		LOG4CXX_INFO(logger, "Writing trace to " << "/tmp/dsgdpp-trace.R");
		trace.toRfile("/tmp/dsgdpp-trace.R", "dsgdpp");
	}
